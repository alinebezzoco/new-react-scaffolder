...
generate.prototype.createComponentFile = function(re, componentName, answersInner) {
	...
	fs.writeFileSync(path.join(process.cwd(), "src", module, componentName + '.js'), __modFile, 'utf-8');
}
...
generate.prototype.generateComponent = function(type, module, componentName, answers, answersInner, cb) {
	try {
		if(componentName !== undefined) {
			const exists = fs.accessSync(path.join(process.cwd(), "src", module, componentName), fs.F_OK);
      cb(`${componentName}.js already exists`);
      return;
		}
		else {
			const exists = fs.accessSync(path.join(process.cwd(), "src", module + '.js'), fs.F_OK);
			cb(`${componentName}.js already exists`);
			return;
		}
	}
		...
		if(componentName !== undefined) {
			try {
				if(answers.propTypes === 'no') {
					...
					const _modFile = file.replace(re, componentName);
					fs.writeFileSync(path.join(process.cwd(), "src", module, componentName + '.js'), _modFile, 'utf-8');
					cb(true);
				}
				else {
					fs.mkdirSync(path.join(process.cwd(), module), 0755);
			...
			try {
				...
				fs.writeFileSync(path.join(process.cwd(), "src", module + '.js'), __modFile, 'utf-8');
				cb(true);
			}
			catch(ex) {
				cb('error');
...
generate.prototype.createTest = function(module, testName, cb) {
	try {
		if(testName !== undefined) {
			const existsModule = fs.accessSync(path.join(process.cwd(), 'tests', "src", module, testName), fs.F_OK);
			cb(`${componentName}.js test file already exists`);
			return;
		}
		else {
			const exists = fs.accessSync(path.join(process.cwd(), 'tests', "src", module + '.js'), fs.F_OK);
			cb(`${componentName}.js test file already exists`);
			return;
		}
	}
		...
		if(testName !== undefined) {
			try {
				...
				fs.writeFileSync(path.join(process.cwd(), 'tests', "src", module, testName + '.js'), file, 'utf-8');
				cb(true);
			}
			catch(ex) {
				if(ex.syscall == 'open') {
			...
			try {
				fs.writeFileSync(path.join(process.cwd(), 'tests', "src", module + '.js'), file, 'utf-8');
				cb(true);
			}
			catch(ex) {
				if(ex.syscall == 'open') {
